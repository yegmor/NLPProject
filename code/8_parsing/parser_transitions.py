...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == "S":
            fw = self.buffer.pop(0)
            self.stack.append(fw)

        elif transition == "LA":
            fw = self.stack[-1]
            sw = self.stack.pop(-2)
            self.dependencies.append((fw, sw))
            
        elif transition == "RA":
            sw = self.stack[-2]
            fw = self.stack.pop(-1)
            self.dependencies.append((sw, fw))

        else:
            print("[ERROR]: No matched transition!")
...
def minibatch_parse(sentences, model, batch_size):
    ...
    dependencies = [complt_parse.dependencies for complt_parse in partial_parses]
